...
void handle_instruction(Instruction_t *instruction, int tick) {
    ...
    if(instruction->burst_time == 0) { 
        ...
        // Add an an instance of task_t to the global task_table
        task_table[task_id-1].id = task_id;
        // Newly arrived task's current queue corresponds to the first queue
        ...
        // TODO ... Insert your Code Here
        // TAT = FT - AT (Turnaround time = Finish time - Arrival time)
        // WT = TAT - PROC (Waiting time = Turnaround time - Processing time)
        ...
        // TODO ... Insert your Code Here
        // Set burst_time and remaining_burst_time to be # of CPU time ticks required for an action 
        // initiated by the task
        task_table[task_id-1].burst_time = instruction->burst_time;
        task_table[task_id-1].remaining_burst_time = instruction->burst_time;
        // enqueue task if a new CPU burst needs to be scheduled
...
Task_t *peek_priority_task() {
    ...
    if (is_empty(queue_1) == 0){
        // return a reference to the task at the start of queue 1 (task with the highest priority)
        return &task_table[queue_1->start->id - 1];
    }
    // check if the second queue is not empty
    else if (is_empty(queue_2) == 0){
        // return a reference to the task at the start of queue 2 (task with the highest priority)
        return &task_table[queue_2->start->id - 1];
    }
    // check if the third queue is not empty
    else if (is_empty(queue_3) == 0){
        // return a reference to the task at the start of queue 3 (task with the highest priority)
...
void boost(int tick) {
    ...
    // Conduct boost
    // TODO - Insert your code here
    // Keep running the while loop as long as queue 3 is not empty
    while (is_empty(queue_3) != 1){
        int tid = queue_3->start->id; // get the task id of the first task in the queue 3
        dequeue(queue_3); // dequeue the first task
        enqueue(queue_1, &task_table[tid-1]); // enqueue the first task from queue 3 into queue 1
        task_table[tid-1].current_queue = 1; // set the current queue for the task to 1
    }
    // Keep running the while loop as long as queue 2 is not empty
    while (is_empty(queue_2) != 1){
        int tid = queue_2->start->id; // get the task id of the first task in the queue 2
        dequeue(queue_2); // dequeue the first task
        enqueue(queue_1, &task_table[tid-1]); // enqueue the first task from queue 2 into queue 1
        task_table[tid-1].current_queue = 1; // set the current queue for the task to 1
    }
    // Check if there exists a current task
    if (current_task != NULL){
        // Check if the remaining quantum for the task is more than 2
        if (remaining_quantum > 2){
            remaining_quantum = 2; // Set the remaining quantum for the ask to 2
        }
        current_task->current_queue = 1; // Set the current queue for the current task to 1 so it's not
                                         // preempted by another task right after the boost 
    }
...
void scheduler() {
    // TODO ... Insert your Code Here
    // Check if the current task has finished its quantum and still needs to have CPU ticks scheduled
    if (current_task != NULL && remaining_quantum == 0 && current_task->remaining_burst_time > 0){
        decrease_task_level(current_task); // Move the current task to a lower queue
        enqueue(get_queue_by_id(current_task->current_queue), current_task); // enqueue the task to the end of its current queue
        current_task = NULL;
    }

    Task_t *priority_task = peek_priority_task();
    // Check if there is a task with a higher priority that can pre-empt the current task
    ...
    // Schedule the next task with the highest priority
    if (current_task == NULL && priority_task != NULL && priority_task->burst_time > 0){
        ...
        // Assign quantum values based on the task's queue
